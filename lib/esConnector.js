...
var ESConnector = function (settings, dataSource) {
  ...
  doa = Object.assign(doa, {
    ...
    bulkDelete: ESConnector.prototype.bulkDelete.bind(this)
  });
...
ESConnector.prototype.dataSourceToModel = function (modelName, data, idName) {
  ...
  if (data.highlight) {
      data._source.highlight = data.highlight;
  }
 ...
 * @returns {Error} - if argument rules are violated
...
ESConnector.prototype.bulk = function bulk(modelName, params, done){
  ...
  if (Array.isArray(params.body) === false || params.body.length <= 0) {
  ...
  self.db
      ...
      .then(
        function(res){
          ...
          if (typeof done === 'function') { done(null, res); }
        },
        function(err){
          console.trace(err.message);
 ...
 * @returns {Error} - if argument rules are violated
 *
 * NOTE: if params has a field called `body`, it gets ignored
 */
ESConnector.prototype.bulkCreate = function bulkCreate(modelName, dataList, params, done) {
  ...
  var bulkParams = Object.assign({}, defaultParams, params);
  ...
  self.bulk(modelName, bulkParams, done);
};
 ...
 * NOTE: if params has a field called `body`, it gets ignored
 */
ESConnector.prototype.bulkDelete = function bulkDelete(modelName, dataList, params, done){
  var self = this;
